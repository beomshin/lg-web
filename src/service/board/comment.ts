...
import FindAnonymAllCommentBoardRequestDto from "@/dto/board/comment/FindAnonymAllCommentBoardRequestDto";
import FindUserAllCommentBoardRequestDto from "@/dto/board/comment/FindUserAllCommentBoardRequestDto";
...
export default {
    ...
    findAnonymAllCommentBoard: (params: FindAnonymAllCommentBoardRequestDto) => { // 게시판 댓글 전체 조회(익명)
        return get(EndPoint.FIND_ANONYM_ALL_COMMENT_BOARD(params.id), ParamsUtils.EMPTY, HeaderUtils.NORMAL);
    },
    findUserAllCommentBoard: (params: FindUserAllCommentBoardRequestDto) => { // 게시판 댓글 전체 조회(회원)
        return get(EndPoint.FIND_USER_ALL_COMMENT_BOARD(params.id), ParamsUtils.EMPTY, HeaderUtils.AUTH_HEADER);
    },
    findAnonymParentCommentBoard: (params: FindAnonymParentCommentBoardRequestDto) => { // 게시판 부모 댓글 조회(익명)
        return get(EndPoint.FIND_ANONYM_PARENT_COMMENT_BOARD(params.id), params, HeaderUtils.NORMAL);
    },
    findUserParentCommentBoard: (params: FindUserParentCommentBoardRequestDto) => { // 게시판 부모 댓글 조회(회원)
        return get(EndPoint.FIND_USER_PARENT_COMMENT_BOARD(params.id), params, HeaderUtils.AUTH_HEADER);
    },
    findAnonymChildrenCommentBoard: (params: FindAnonymChildrenCommentBoardRequestDto) => { // 게시판 자식 댓글 조회(익명)
        return get(EndPoint.FIND_ANONYM_CHILDREN_COMMENT_BOARD(params.id), params, HeaderUtils.NORMAL);
    },
    findUserChildrenCommentBoard: (params: FindUserChildrenCommentBoardRequestDto) => { // 게시판 자식 댓글 조회(회원)
