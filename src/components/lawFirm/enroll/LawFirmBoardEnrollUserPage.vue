...
export default {
  name: "LawFirmBoardEnrollUserPage",
  setup () {
    const router = useRouter()
    const route = useRoute()
    const lawfirmId = Number(route.query.id)
    const title = ref('')
    const content = ref('')
  ...
  methods: {
    EnrollBoard () {
      if (!this.validateBoard()) {
        ...
        service
            .LawFirmBoardEnroll(
                new LawFirmBoardEnroll(this.lawfirmId, this.title, this.content, this.lineType, this.files),
                {
            ...
            .then(res => {
              if (res.data.resultCode == '00000') {
                alert('로펌 게시글 등록에 성공했습니다.')
                window.history.back()
              } else if (res.data.resultCode == '10018') {
                alert('해당 로펌 유저가 아닙니다.')
              } else {
                alert('로펌 게시글 등록에 실패했습니다.')
              }
            })
            .catch(err => {
              console.log(err)
